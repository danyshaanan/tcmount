...
function linkWithSuffix(suffix) {
    ...
    for (var i in links) {
        var link = links[i];
        if ((link.file && link.mountpoint) && (isSuffixOf(suffix, link.file) || isSuffixOf(suffix, link.mountpoint))) {
            return link;
        }
    }
    return null;
...
function guessLink(hint) {
    ...
    if (fs.existsSync(location)) {
        var bySuffix = linkWithSuffix(location);
        if (bySuffix) {
            return bySuffix;
        } else {
            return location;
    ...
    var bySuffix = linkWithSuffix(hint);
    if (bySuffix) {
        return bySuffix;
    }
